...
import type { CreateSessionRequest, CreateSessionResponse, Env } from "./types";
...
const SANDBOX_AUTH_ROUTES: RegExp[] = [
  /^\/sessions\/[^/]+\/ws$/, // WebSocket connection from sandbox
...
const routes: Route[] = [
  // Health check
  {
    ...
    handler: async () => json({ status: "healthy", service: "dispatch-control-plane" }),
  },
  ...
  {
    method: "POST",
    pattern: parsePattern("/sessions/:id/question/:requestId/reply"),
    handler: handleQuestionReply,
  },
...
async function handleDeleteSession(
  ...
  // Call cleanup on the Durable Object to terminate sandbox and clear state
  const doId = env.SESSION.idFromName(sessionId);
  const stub = env.SESSION.get(doId);

  try {
    await stub.fetch(
      new Request("http://internal/internal/cleanup", {
        method: "POST",
      })
    );
  } catch (e) {
    console.warn(`[router] Cleanup failed for session ${sessionId}:`, e);
...
async function handleQuestionReply(
  ...
  if (!sessionId || !requestId) return error("Session ID and Request ID required");
  ...
  return stub.fetch(
    new Request("http://internal/internal/question-reply", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ requestId, answers: body.answers }),
    })
  );
}
