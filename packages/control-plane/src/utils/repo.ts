 ...
 * and comparison of repository identifiers across the control plane.
 */
...
/**
 * Get normalized metadata key for KV storage.
 *
 * @param owner - Repository owner
 * @param name - Repository name
 * @returns KV key in the format "repo:metadata:owner/name"
 */
export function getRepoMetadataKey(owner: string, name: string): string {
  return `repo:metadata:${normalizeRepoId(owner, name)}`;
}
