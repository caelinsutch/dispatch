...
export class SessionDO extends DurableObject<Env> {
  ...
  private readonly routes: InternalRoute[] = [
    ...
    {
      method: "POST",
      path: "/internal/question-reply",
      handler: (req) => this.handleQuestionReply(req),
    },
    {
      method: "POST",
  ...
  private async restoreFromSnapshot(snapshotImageId: string): Promise<void> {
    ...
    try {
      ...
      // Get control plane URL from env var (set via wrangler.jsonc or Terraform)
      const controlPlaneUrl = this.env.WORKER_URL;
      if (!controlPlaneUrl) {
        throw new Error("WORKER_URL environment variable not configured");
      }
  ...
  private async spawnSandbox(): Promise<void> {
    ...
    try {
      ...
      // Get the control plane URL from env var (set via wrangler.jsonc or Terraform)
      const controlPlaneUrl = this.env.WORKER_URL;
      if (!controlPlaneUrl) {
        throw new Error("WORKER_URL environment variable not configured");
      }
      ...
      // Store Modal's internal object ID and tunnel URLs
      if (result.modalObjectId || result.tunnelUrls) {
        this.sql.exec(
          `UPDATE sandbox SET modal_object_id = ?, tunnel_urls = ? WHERE id = (SELECT id FROM sandbox LIMIT 1)`,
  ...
  private getSessionState(): SessionState {
    ...
    let tunnelUrls: Record<number, string> | undefined;
    if (sandbox?.tunnel_urls) {
      try {
        tunnelUrls = JSON.parse(sandbox.tunnel_urls);
      } catch (e) {
        console.error("[DO] Failed to parse tunnel_urls:", e);
      }
    }
  ...
  /**
   * Handle question reply from user - forward to sandbox.
   */
  private async handleQuestionReply(request: Request): Promise<Response> {
    const { requestId, answers } = (await request.json()) as {
      requestId: string;
      answers: string[][];
    };
  ...
  /**
   * Handle session cleanup - terminates sandbox and clears data.
   * Called when a session is deleted.
   */
  private async handleCleanup(): Promise<Response> {
    console.log("[DO] Cleanup: terminating sandbox and clearing session data");

    // Send shutdown to sandbox if connected
    const sandboxWs = this.getSandboxWebSocket();
    if (sandboxWs) {
      console.log("[DO] Cleanup: sending shutdown to sandbox");
      this.safeSend(sandboxWs, { type: "shutdown" });
      try {
        sandboxWs.close(1000, "Session deleted");
      } catch {
        // Ignore errors closing WebSocket
      }
    }
    ...
    const clientSockets = this.ctx.getWebSockets().filter((ws) => {
      const tags = this.ctx.getTags(ws);
      return !tags.includes("sandbox");
    });
    ...
    this.sql.exec(`UPDATE session SET status = 'deleted', updated_at = ?`, Date.now());

    // Cancel any scheduled alarms
    try {
      await this.ctx.storage.deleteAlarm();
    } catch {
      // Ignore if no alarm was scheduled
    }

    console.log("[DO] Cleanup: complete");
    return Response.json({ status: "cleaned_up" });
  }
