"""
Sandbox lifecycle management for Modal Sandbox.
...
class SandboxHandle:
    ...
    tunnel_urls: dict[int, str] | None = None  # URLs for exposed ports via Modal Tunnel
...
class SandboxManager:
    """
    Manages sandbox lifecycle for Modal Sandbox sessions.
    ...
    async def create_sandbox(
        ...
        if config.snapshot_id:
            ...
            image = modal.Image.from_registry(f"dispatch-snapshot:{config.snapshot_id}")
        else:
        ...
        sandbox = modal.Sandbox.create(
            ...
            workdir="/workspace",
            env=env_vars,
            encrypted_ports=PREVIEW_PORTS,  # Expose common dev ports via Modal Tunnel
        )
        ...
        # Get tunnel URLs for exposed ports (wait up to 120s for tunnels to be ready)
        tunnel_urls: dict[int, str] = {}
        try:
            print(f"[manager] Getting tunnel URLs for ports: {PREVIEW_PORTS}")
            tunnels = sandbox.tunnels(timeout=120)
            print(f"[manager] Got {len(tunnels)} tunnels")
            for port, tunnel in tunnels.items():
                tunnel_urls[port] = tunnel.url
                print(f"[manager] Tunnel for port {port}: {tunnel.url}")
        except Exception as e:
            import traceback
            print(f"[manager] Warning: Could not get tunnel URLs: {e}")
            traceback.print_exc()
