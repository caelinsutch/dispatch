...
class AgentBridge:
    ...
    def __init__(
        ...
        # Set repo_path based on repo_name if provided
        if repo_name:
            self.repo_path = Path("/workspace") / repo_name
            print(f"[bridge] Using repo_path: {self.repo_path} (repo_name={repo_name})")
        else:
            self.repo_path = Path("/workspace")
            print(f"[bridge] Using default repo_path: {self.repo_path} (no repo_name provided)")
    ...
    async def _handle_command(self, cmd: dict[str, Any]) -> asyncio.Task[None] | None:
        ...
        elif cmd_type == "question_reply":
            await self._handle_question_reply(cmd)
        else:
    ...
    def _transform_part_to_event(
        ...
        if part_type == "text":
            ...
            # DEBUG: Log full structure for question tool to find OpenCode question ID
            if part.get("tool") == "question":
                import json
                print("[bridge] DEBUG QUESTION TOOL - All part keys:", list(part.keys()))
                print("[bridge] DEBUG QUESTION TOOL - part.id:", part.get("id"))
                print("[bridge] DEBUG QUESTION TOOL - part.callID:", part.get("callID"))
                print("[bridge] DEBUG QUESTION TOOL - part.tool:", part.get("tool"))
                print("[bridge] DEBUG QUESTION TOOL - part.type:", part.get("type"))
                print("[bridge] DEBUG QUESTION TOOL - state keys:", list(state.keys()) if state else "None")
                print("[bridge] DEBUG QUESTION TOOL - state.id:", state.get("id") if state else "None")
                print("[bridge] DEBUG QUESTION TOOL - state.requestID:", state.get("requestID") if state else "None")
                print("[bridge] DEBUG QUESTION TOOL - state.input keys:", list(tool_input.keys()) if tool_input else "None")
                print("[bridge] DEBUG QUESTION TOOL - state.input.id:", tool_input.get("id") if tool_input else "None")
                print("[bridge] DEBUG QUESTION TOOL - Full part dict:")
                print(json.dumps(part, indent=2, default=str))
                print("[bridge] DEBUG QUESTION TOOL - Full state dict:")
                print(json.dumps(state, indent=2, default=str))

            # Include part ID in args for question tool (OpenCode question ID)
    ...
    async def _stream_opencode_response_sse(
        ...
        # Use /event endpoint (NOT /global/event which closes immediately)
                    ...
                    async for event in self._parse_sse_stream(sse_response):
                        ...
                        if event_type == "question.asked":
                            ...
                            if question_id:
                                ...
                                print(f"[bridge] question.asked: captured question ID {question_id}")
                            continue
                        ...
                        if event_type == "message.part.updated":
                            ...
                            if part_type == "text":
                                ...
                                if tool_event:
                                    state = part.get("state", {})
                                    # Add question ID to question tool calls
                                    if tool_event.get("tool") == "question" and current_question_id:
                                        tool_event["args"]["id"] = current_question_id
                                        print(f"[bridge] Added question ID {current_question_id} to tool_call args")
    ...
    async def _handle_question_reply(self, cmd: dict[str, Any]) -> None:
        """Handle question reply command - send reply and stream continuation.

        After answering a question, OpenCode continues processing and emits events
        on the SSE stream. We need to subscribe to SSE and forward those events
        to the control plane.
        """
        request_id = cmd.get("requestId", "")
        answers = cmd.get("answers", [])
        # Use provided messageId or generate one for event correlation
        message_id = cmd.get("messageId") or f"qreply_{request_id}"

        if not self.http_client:
            print("[bridge] Cannot reply to question: HTTP client not initialized")
            return

        url = f"{self.opencode_base_url}/question/{request_id}/reply"
        print(f"[bridge] Sending question reply to: {url}")
        print(f"[bridge] Answers: {answers}")

        try:
            response = await self.http_client.post(
                url,
                params={"directory": str(self.repo_path)},
                json={"answers": answers},
                timeout=30.0,
            )
            print(f"[bridge] Question reply response: {response.status_code}")

            if response.status_code != 200:
                print(f"[bridge] Question reply failed: {response.status_code} - {response.text}")
                await self._send_event(
                    {
                        "type": "error",
                        "error": f"Question reply failed: {response.status_code}",
                        "messageId": message_id,
                    }
                )
                return
            ...
            print(f"[bridge] Error in question reply: {e}")
            await self._send_event(
                {
                    "type": "execution_complete",
                    "messageId": message_id,
    ...
    async def _stream_sse_continuation(
        self,
        message_id: str,
    ) -> AsyncIterator[dict[str, Any]]:
        """Stream SSE events after question reply or other continuation.

        Similar to _stream_opencode_response_sse but without sending a prompt.
        Subscribes to the global event stream and processes events until
        session.idle is received.

        Args:
            message_id: Control plane message ID for events sent back
        """
        if not self.http_client or not self.opencode_session_id:
            raise RuntimeError("OpenCode session not initialized")
        ...
        # Use /event endpoint (NOT /global/event which closes immediately)
        sse_url = f"{self.opencode_base_url}/event"
        print(f"[bridge] Connecting to SSE for continuation: {sse_url}")
        ...
        try:
            async with asyncio.timeout(max_wait_time):
                async with self.http_client.stream(
                    ...
                    print("[bridge] SSE connected for continuation, processing events...")
                    ...
                    async for event in self._parse_sse_stream(sse_response):
                        ...
                        if event_type == "message.updated":
                            ...
                            if msg_session_id == self.opencode_session_id:
                                ...
                                print(
                                    f"[bridge] continuation message.updated: role={role}, "
                                    f"id={oc_msg_id}, finish={finish}"
                                )

                                # Track all assistant messages in our session during continuation
                                if role == "assistant" and oc_msg_id:
                                    our_assistant_msg_ids.add(oc_msg_id)
                                    print(
                                        f"[bridge] Tracking assistant message {oc_msg_id} during continuation"
                                    )
                        ...
                        if event_type == "message.part.updated":
                            ...
                            if idle_session_id == self.opencode_session_id:
                                elapsed = time.time() - start_time
                                print(
                                    f"[bridge] SSE continuation session.idle received after {elapsed:.1f}s"
                                )
                                return
                            ...
                            if (
                                ...
                                elapsed = time.time() - start_time
                                print(
                                    f"[bridge] SSE continuation session.status idle received after {elapsed:.1f}s"
                                )
                                return
                            ...
                            if error_session_id == self.opencode_session_id:
                                ...
                                print(f"[bridge] SSE continuation session.error: {error_msg}")
                                yield {
                                    "type": "error",
                                    "error": error_msg or "Unknown error",
                                    "messageId": message_id,
            ...
            elapsed = time.time() - start_time
            print(f"[bridge] SSE continuation stream timed out after {elapsed:.1f}s")
            raise RuntimeError("Continuation stream timed out")

        except httpx.ReadError as e:
            print(f"[bridge] SSE continuation read error: {e}")
            raise SSEConnectionError(f"SSE read error: {e}")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Modal Sandbox Agent Bridge")
    ...
    parser.add_argument("--repo-name", help="Repository name (for setting repo_path)")
