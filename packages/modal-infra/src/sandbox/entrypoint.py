...
class SandboxSupervisor:
    ...
    async def start_opencode(self) -> None:
        ...
        instructions = (
            ...
            "**Branch Naming:**\n"
            "- You can rename your branch to something more descriptive using `git branch -m <new-name>`\n"
            "- Good branch names describe the feature/fix: `feature/add-dark-mode`, `fix/login-button`\n"
            "- The PR tool will use whatever branch name you've set\n\n"
            "**Committing Changes:**\n"
            ...
            "- Don't switch to other branches (main, master, etc.) - stay on your session branch\n"
            "- The base branch for PRs defaults to the repo's default branch\n\n"
            "## Live Preview\n\n"
        ...
        # Pass through AWS credentials for Bedrock provider
        # These are set in the llm-api-keys secret
        # Supports both bearer token auth (AWS_BEARER_TOKEN_BEDROCK) and IAM credentials
        for aws_key in ["AWS_BEARER_TOKEN_BEDROCK", "AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY", "AWS_REGION"]:
            if aws_key in os.environ:
                env[aws_key] = os.environ[aws_key]
