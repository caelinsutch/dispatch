#!/usr/bin/env node
/**
 * Skill Evaluation Engine for Dispatch
 *
...
function extractFilePaths(prompt) {
	...
	const extensionPattern =
		/(?:^|\s|["'`])([\w\-./]+\.(?:[tj]sx?|json|css|md|sh|py))\b/gi;
	...
	const dirPattern =
		/(?:^|\s|["'`])((?:packages|apps|src|components|\.claude|\.github)\/[\w\-./]+)/gi;
...
function evaluate(prompt) {
	...
	output += "SKILL ACTIVATION RECOMMENDED\n\n";
	...
	output += "1. EVALUATE: Consider if each skill is relevant to the task\n";
	output += "2. ACTIVATE: Invoke the Skill tool for relevant skills\n";
	output += "3. IMPLEMENT: Apply skill guidance during implementation\n";
	output += "\nExample: To activate vercel-react-best-practices, use /vercel-react-best-practices\n";
...
function main() {
	...
	process.stdin.on("end", () => {
		...
		try {
			...
			// Silent failure to not block prompts
		}
